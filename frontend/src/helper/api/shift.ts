...
export const getShifts = async (week: number, year: number) => {
  const api = getAxiosInstance()
  const { data } = await api.get(`/shifts?order[date]=DESC&order[startTime]=ASC&where[week]=${week}&where[year]=${year}`);
  return data;
};
